# -*- coding: utf-8 -*-
"""设备发现与 APP 包名自动检测 —— 全部基于 adb 命令,不依赖 uiautomator2 连接。"""
import os
import re
import subprocess
import time
...
except ImportError:  # 未安装时退化为仅原名匹配(ASCII 名称不受影响,中文名会匹配不到)
    def lazy_pinyin(s):
        return [s]

# Windows 上 subprocess 默认给子进程新建控制台窗口 —— GUI 里每调一次 adb
# 就会闪一个黑框(点开步骤卡片之类本该纯界面操作的地方也会闪)。
...
def _ensure_adb_server():
    """预热 adb server:GUI 启动后第一次 adb devices 要拉起 server,
    冷启动可能超过 list_devices 的超时(实测"adb 检测失败"的根源)"""
    global _server_ready
    if _server_ready:
        return
    try:
        subprocess.check_output(["adb", "start-server"], timeout=30,
                                stdin=subprocess.DEVNULL,
                                stderr=subprocess.STDOUT,
                                creationflags=_NO_WINDOW)
    except Exception as e:
        log.warning(f"[app_detect] adb start-server 异常(忽略): {e}")
    _server_ready = True

# 显示名与包名对不上的 APP 别名表(切词切不出共同子串的情况)
_APP_ALIASES = {
    "smartthings": {"oneconnect"},
}


def _adb_out(args, timeout=8):
    """adb 子进程统一封装。

    ★ pythonw(GUI)进程没有控制台,stdin/stderr 句柄都是无效的;子进程继承
    任一无效句柄都会让 CreateProcess 直接抛 [WinError 50] 不支持该请求 ——
    这就是 GUI 里"adb 检测失败"而 pytest(python.exe)一切正常的根源。
    因此 stdin 必须指向 DEVNULL、stderr 必须并入 stdout,两个都要显式设置
    (只修 stderr 不修 stdin 时报错会间歇性复现)。
    """
...
def _adb_shell(device_id, *args, timeout=8):
    ...
    return _adb_out(cmd, timeout=timeout)


def list_devices():
    """adb devices → [{'id':..., 'state':'device'|'offline'|...}] 含真机与模拟器

    首次调用会拉起 adb server(冷启动慢)——超时自动重试一次,仍失败才抛给上层。
    """
    _ensure_adb_server()
    out, last = None, None
    for _ in range(2):
        try:
            out = _adb_out(["adb", "devices"], timeout=15)
            break
        except Exception as e:
            last = e
            time.sleep(2)
...
def _parse_brief_activity(out, package):
    """从 resolve-activity --brief 输出里取启动页"""
